...
class YT_Minify extends JObject
{
	...
	function __construct()
	{
		...
		$this->optimizeMergeFile	= $app->getTemplate(true)->params->get('optimizeMergeFile', 1);
	...
	function setMinifyConfigFile()
	{
		...
		$content = JFile::read(JPATH_PLUGINS.DS."system".DS."yt".DS."includes".DS."libs".DS."minify".DS."config_default.php");
		$content = explode("\n", $content);
		
		if(stristr(PHP_OS, 'WIN'))
		{
			$path1 = str_replace(DS, "/", JPATH_ROOT.DS);
			$path2 = str_replace(DS, "/", JPATH_ROOT.DS.$this->optimizeFolder.DS);
		}
		else
		{
			$path1 = JPATH_ROOT.DS;
			$path2 = JPATH_ROOT.DS.$this->optimizeFolder.DS;
		}
		...
		JFile::write(JPATH_PLUGINS.DS."system".DS."yt".DS."includes".DS."libs".DS."minify".DS."config.php", implode("\n", $content));
	}
