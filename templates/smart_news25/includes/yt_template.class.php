...
class YtTemplate {
	...
	function YtTemplate ($template =null) {
		...
		if(!isset($_params_cookie)) {
			$_params_cookie = array();
		}
		$arr_param =array('switch_modes',
						  'direction',
		...
		for($i=0; $i<count($arr_param); $i++){
			if( !in_array($this->getParam($arr_param[$i]), $_params_cookie)) {
				$_params_cookie[]=  $arr_param[$i];  
			}
		}
	...
	function getControlFontSize(){
        ...
        <ul class="yt-fontsize clearfix">
            ...
            <li class="dec" style="cursor: pointer;" title="Decrease Text Size" onclick="switchFontSize('<?php echo $this->template."_fontsize";?>','dec'); return false;"><span>Decrease</span></li>
            <li class="reset" title="Reset Text Size" style="cursor: pointer;" onclick="switchFontSize('<?php echo $this->template."_fontsize";?>',<?php echo $this->_tpl->params->get('fontsize');?>); return false;"><span>Reset</span></li>
            <li class="inc" title="Increase Text Size" style="cursor: pointer;" onclick="switchFontSize('<?php echo $this->template."_fontsize";?>','inc'); return false;"><span>Increase</span></li>
         </ul>
	...
	function renPositionsGroup($position, $type=''){	
		...
		if ( $position['type'] == 'html' ) {
			...
			if($position['value'] == '@fontsize'){
				$class .= ($doc->countModules('breadcrumb')?' show-breadcrumb':' hide-breadcrumb');
			}
		}else{
			$class = "";
		}
		...
		if ( $position['type'] == 'modules' ) { 
			...
			if ( $has_suffix ) {
				$this->renModulePos($position['value'], $position['style'], $elStyle, '', $position['value'] . '-' . $this->getParam('layoutsuffix'));
			} else {
	...
	function renModulePos ($positionname, $modulestyle, $elementstyle, $elementclass='', $positionnamesuffix='' ) {
		?>
	...
	function renComponent ($elementstyle) {
		?>
        <div id="yt_component"<?php echo $elementstyle; ?>>
