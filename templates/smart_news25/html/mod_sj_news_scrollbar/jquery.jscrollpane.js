		...
		{ 
			...
			function initialise(s)
			{
				...
				if (pane == undefined) {
                    ...
                    var arrImage = elem.children('div.item_scrollbar');
                   // var arrtooltip = elem.children('div.stickytooltip_theme3').height();
                    ...
                    //alert(maxHeight);
                    if(settings.width_module == 100){
                        settings.width_module = paneWidth;
                    }else{
                        settings.width_module = settings.width_module;
                    }
                    var moreHeight = (settings.rownumber * 20) + 25;
                    var moreWidth = (settings.item_per_page * 25);
                    //alert(settings.rownumber + moreHeight);
					elem.width(paneWidth);
					pane = $('<div class="jspPane" />').wrap(
						$('<div class="jspContainer" />')
                            .css({
								'width': (paneWidth) + 'px',
								'height': ((maxHeight * settings.rownumber) + 10/*moreHeight*/) + 'px'
							})
					);
                    //alert((maxWidth * settings.item_per_page));
                   // elem.hide();
				...
				if (isScrollableH) {
					...
					container.append(
						$('<div class="jspHorizontalBar" />').append(
							...
							$('<div class="yt-title-scroll">Special News</div>'),
							$('<div class="jspTrack" />').append(
								$('<div class="jspDrag" />').append(
					...
					horizontalTrackWidth = paneWidth - 162;
					sizeHorizontalScrollbar();
                    //alert(horizontalTrackWidth);
				} else {
			...
			function resizeScrollbars()
			{
				...
				// reflow content
				if (isScrollableH) {
					pane.width(settings.thumWidth + 'px');
				}
                //alert(settings.thumWidth);
				contentHeight = pane.outerHeight();
				percentInViewV = contentHeight / paneHeight;
